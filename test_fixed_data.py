...
def test_fixed_data(dataset_path, model_path, save_dir="outputs"):
    """测试修复后的数据集"""
    ...
    output_path = os.path.join(save_dir, "prediction_results_fixed.csv")
    ...
    # 生成可视化图表
    create_test_plots(df, all_y_true.flatten(), all_y_pred.flatten(), save_dir)
    
    return df, r2, pearson

def create_test_plots(df, y_true, y_pred, save_dir):
    """创建测试结果的可视化图表"""
    print("🎨 生成测试结果可视化图表...")
    ...
    plt.title(f'kcat: True vs Predicted (R² = {r2:.3f}) - Fixed Data')
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'kcat_prediction_scatter_fixed.png'), dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.xlabel('True kcat (log10)')
    plt.ylabel('Predicted kcat (log10)')
    plt.title('kcat: Density Plot - Fixed Data')
    plt.savefig(os.path.join(save_dir, 'kcat_density_fixed.png'), dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.title('Residual Plot - Fixed Data')
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'kcat_residuals_fixed.png'), dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'kcat_comprehensive_analysis_fixed.png'), dpi=300, bbox_inches='tight')
    plt.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default="kcat_test_fixed.pt", help='Path to .pt dataset')
    parser.add_argument('--model', type=str, default='outputs/fixed_data_training/best_model.pt', help='Path to model')
    parser.add_argument('--save_dir', type=str, default='outputs/predictions/fixed_data_test', help='Output directory')
    args = parser.parse_args()
    
    print("🚀 开始测试修复后的数据集...")
    test_fixed_data(args.dataset, args.model, args.save_dir)
