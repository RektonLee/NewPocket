...
"""
创建与原始数据格式一致的预测结果文件
只保留预测成功的样本，将predicted_km_log10替换为我们的预测值
"""
...
def create_prediction_dataset(original_csv, predictions_csv, output_csv):
    """
    创建与原始数据格式一致的预测结果文件
    ...
    for _, pred_row in successful_predictions.iterrows():
        ...
        if len(original_row) > 0:
            ...
            new_row = {
                ...
                'predicted_km_log10': pred_row['km_pred_log10']  # 使用我们的预测值
            }
    ...
    # 显示预测值范围
    print(f"预测值范围 (log10): {result_df['predicted_km_log10'].min():.3f} 到 {result_df['predicted_km_log10'].max():.3f}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 4:
        print("用法: python create_prediction_dataset.py <原始数据CSV> <预测结果CSV> <输出CSV>")
        print("示例: python create_prediction_dataset.py km_test_data_with_values.csv merged_predictions.csv prediction_dataset.csv")
        sys.exit(1)
    ...
    result = create_prediction_dataset(original_csv, predictions_csv, output_csv)
    
    print(f"\n✅ 完成！生成了包含 {len(result)} 个成功预测样本的数据集")
